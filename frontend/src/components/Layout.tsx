// File updated: April 18, 2025
// Added AI Stylist Co-Pilot global float + SkinTone context
...
const Layout = ({ children }: LayoutProps) => {
  ...
  return (
    <div className="min-h-screen bg-background antialiased overflow-x-hidden relative">
      ...
      <Footer />
      {/* Global AI Stylist Co-Pilot — floats on every page */}
      <AIStylistWrapper />
    </div>
  );
};

// AIStylistWrapper reads from global skin tone context and passes it down
const AIStylistWrapper = () => {
  const { skinToneData } = useSkinTone();
  return <AIStylist skinToneContext={skinToneData} />;
};
